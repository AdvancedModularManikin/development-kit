...
sudo apt update -y
sudo apt install -y openocd cmake default-jdk gradle libboost-all-dev libtinyxml2-dev gcc-arm-none-eabi rapidjson-dev doxygen sqlite3 libsqlite3-dev sqlite libsqlite-dev man-db git-lfs
git lfs install
...
git clone https://github.com/EntropicEngineering/biogears_aarch64.git
pushd biogears_aarch64
echo "Installing Biogears"
sudo tar xzf biogears.tar.gz -C /opt
